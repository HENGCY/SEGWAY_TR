...
uint8_t TX_USART [54] = "M0:+0000M1:+0000M2:+0000M3:+0000M4:+0000RX:0000\r"; 		
uint8_t RX_USART [7]   = "0000000"; 																		
uint8_t ST[10]   = "XXXXXXXXXX"; 				//	test RX serie
...
{	
  // ADC Channel configuration, GPIOC &  GPIOA Periph clock enable 
...
/**
* @brief Initialisations et configuration Timer 1 (PWM).
* @param None
*/
void TIM_Config(void)
{
  ...
  TIM_Cmd(TIM1, ENABLE); 																							//	 TIM1 counter enable 
	...
	/***********************************************************************************************************************/
	// TIMER 2 est activé :
	/* CONFIGURATION DU TIMER 2(50 Hz)*/
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2 , ENABLE);//TIM2 actif
  TimerPeriod = (SystemCoreClock / (50 * prescaler_TIM2)) - 1; //
	TIM_TimeBaseStructure.TIM_Prescaler = prescaler_TIM2 -1; // 0 donne 1
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseStructure.TIM_Period = TimerPeriod;
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
