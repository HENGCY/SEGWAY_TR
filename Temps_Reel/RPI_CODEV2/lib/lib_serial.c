...
int bytes_to_array_of_message(unsigned char * mesg, message_serial *m){
	...
	for (y=0;y<=MESSAGE_SERIAL_LENGTH_READ;y=y+7){
		if(mesg[y] == '<' && mesg[y+6] == '\n' ){
			m[i].label = mesg[y+1];
...
message_serial*  read_from_serial(){
	...
	while (message_length<MESSAGE_SERIAL_LENGTH_READ)
	{
		// Read up to 255 characters from the port if they are there
		unsigned char rx_buffer[256];
		int rx_length = read(uart0_filestream, (void*)rx_buffer, MESSAGE_SERIAL_LENGTH_READ - message_length);		//Filestream, buffer to store in, number of bytes to read (max)
